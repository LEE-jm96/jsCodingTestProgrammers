...
function solution(keymap, targets) {
    ...
    for(let i = 0; i < targets.length; i++){
        ...
        let clickNum = findClickNum(keymap, target);
        if(clickNum === 999){
            answer.push(-1);
        }else{
            answer.push(clickNum);
        }
    }
...
const findClickNum = (keymap, target) => {
    let sum = 0;
    let idx = 0;
    let temp = [];
    ...
    for(let i = 0; i < target.length; i++){
        keymap.map(item => {
            if(item.includes(target[i])){
                idx = item.indexOf(target[i]) + 1;
                temp.push(idx);
            }else{
                temp.push(999);
            }  
