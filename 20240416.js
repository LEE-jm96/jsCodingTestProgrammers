function solution(want, number, discount) {
    const wantProduct = [];
    const numSum = number.reduce((acc, cur) => acc + cur);
    let answer = 0;
    
    number.map((number, idx) => {
        for (let i = 0; i < number; i++){
            wantProduct.push(want[idx]);
        }
    });

    wantProduct.sort();
    
    for(let i = 0; i < discount.length - numSum + 1; i++) {
        if (wantProduct.toString() === discount.slice(i, i + 10).sort().toString()) {
            answer++;
        }
    }
...
/*
문제 설명
XYZ 마트는 일정한 금액을 지불하면 10일 동안 회원 자격을 부여합니다. XYZ 마트에서는 회원을 대상으로 매일 한 가지 제품을 할인하는 행사를 합니다. 할인하는 제품은 하루에 하나씩만 구매할 수 있습니다. 
