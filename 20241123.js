function solution(friends, gifts) {
    const receiveGift = []; // 받을 선물의 개수
    const giftInfo = {}; // 선물 주고 받은 기록
    const giftScore = {}; // 선물 지수
    
    friends.forEach((friend, idx, arr) => {
        giftInfo[friend] = {};
        giftScore[friend] = 0;
        const filterArr = arr.filter(data => data !== friend);
        
        for (let i = 0; i < filterArr.length; i++) {
            giftInfo[friend][filterArr[i]] = 0;
        }
    })
    
    gifts.forEach(gift => {
        const [send, receive] = gift.split(" ");
        giftInfo[send][receive]++;
        giftScore[send]++;
        giftScore[receive]--;
    })
    // ---- 선물 주고 받은 기록, 선물 지수 셋팅 완료 ----
    
    const giftEntries = Object.entries(giftInfo);

    giftEntries.forEach(giftEntry => {
        const [sender, sendObj] = giftEntry;
        const sendEntry = Object.entries(sendObj);
        let receiveGiftNum = 0;
        
        for (let i = 0; i < sendEntry.length; i++) {
            const [receiver, receiveCnt] = sendEntry[i];
