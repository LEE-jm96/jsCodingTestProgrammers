...
모든 신호등은 항상 초록불 → 노란불 → 빨간불 순서로 반복되며, 각 신호의 지속 시간은 신호등마다 다릅니다. 시간은 1초부터 시작하며, 각 신호등은 처음에는 초록불 상태로 시작합니다.
...
function solution(signals) { 
    const lightCount = signals.length;
    const cycles = signals.map(([greenTime, yellowTime, redTime]) =>
        greenTime + yellowTime + redTime
    );

    const totalCycle = getLCMOfArray(cycles);
    const timeline = Array.from({ length: totalCycle + 1 }, () => 0);

    signals.forEach(([greenTime, yellowTime, redTime]) => {
        let start = greenTime + 1;
        let end = greenTime + yellowTime;

        while (start <= totalCycle) {
            timeline[start] += 1;
            timeline[end + 1] -= 1;

            const jump = greenTime + redTime + 1;
            start = end + jump;
            end = start + yellowTime - 1;
        }
    });

    for (let time = 1; time <= totalCycle; time++) {
        timeline[time] += timeline[time - 1];

        if (timeline[time] === lightCount) {
            return time;
        }
    }
